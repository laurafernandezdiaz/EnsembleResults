...
from IEWGS.my_grid_combine import my_grid_combine
...
cv=3
rep=1
grid_mean=my_grid_combine(MLS,GSparams,RG,scoringName,cv,exp,rep,X,y)


sumfit=0
sumpred=0
for i in range(0,rep):
	sumfit=sumfit+grid_mean.grid_combine_models[i].final_time_fit
	sumpred=sumpred+grid_mean.grid_combine_models[i].final_time_predict
print("Time fit per rep",sumfit/rep)
print("Time predict per rep",sumpred/rep)

path=pathlib.Path(__file__).parent.absolute()
# Open file to save grid
with open(str(path)+'\\'+str(exp)+'\\grid_regression_generalisedInv_'+str(dataname)+'_'+str(MLSName)+'_'+str(scoringName)+'_'+str(cv)+'_'+str(exp), 'wb') as file:
 
 
  # Save grid in disk
  pickle.dump(grid_mean, file)
