...
class bayesian_automl:
    ...
    def fit(self,X,y):
        ...
        if os.path.isfile(self.path+'preds_'+str(self.fold)+'.csv'):
            ...
            self.model=self.grid_bayesian[0]
            self.model.fit(self.X_train,self.y_train)
            self.estimators_fit.append(self.model)
    ...
    def predict(self,X):
        ...
        self.y_pred_results=self.estimators_fit[0].predict(self.X_test)#.reshape(self.X_test.shape[0],1) #Predict with the estimator of i rep, j param and this fold  
