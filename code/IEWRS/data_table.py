...
class data_table:
    ...
    def __init__(self,MLS,X,y,GSparams,RG,scoring,total_params,rep,random_params=5,cv_split=2,exp=1):
        """
        Params:
            - MLS    : estimator type
    ...
    def fit(self):
        ...
        self.clf = RandomSearchCV_coef(self.MLS, self.ids_X, self.GSparams,n_iter=self.random_params, cv=self.cv,scoring=self.scoring,iid=False,verbose=0,cv_split=self.cv,exp=self.exp,rep=self.rep)#rep=i)
        self.clf.fit(self.ids_X, self.y)
    
        self.best_estimator_grid_=self.clf.best_estimator_
