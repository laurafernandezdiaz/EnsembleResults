...
from .randomsearchcv_coef import RandomSearchCV_coef
import numpy as np
import sqlite3
...
def sql_connection(exp,rep):
    ...
    try:
        ...
        con = sqlite3.connect('mydatabase_grid_searchC_'+str(exp)+'_'+str(rep)+'.db')
...
class data_table:
    ...
    def __init__(self,MLS,X,y,GSparams,RG,scoring,total_params,random_params,rep,cv,exp=1):
        """
        Params:
            - MLS    : estimator type
    ...
    def fit(self):
        ...
        self.clf = RandomSearchCV_coef(self.MLS, self.ids_X, self.GSparams, cv=self.cv,scoring=self.scoring,iid=False,verbose=0,random_params=self.random_params,cv_split=self.cv,exp=self.exp,rep=self.rep)#,rep=i)
        ...
        n_params_all = int(self.random_params) #Number of total values of params #self.params
        ...
        for j in self.data_rep[:,0]: #For each example of the selected repetition (for each id)
            ...
            #Concatenate the reps,ids,folds, Xs, the predicted evaluations and the real evaluation  
