...
from RandomSearch.my_random_search import my_random_search
import pickle
...
cv=3
rep=1
random_search=my_random_search(MLS,GSparams,RG,scoringName,cv,exp,rep,X,y)


sumfit=0
sumpred=0
for i in range(0,rep):
	sumfit=sumfit+random_search.random_search_models[i].final_time_fit
	sumpred=sumpred+random_search.random_search_models[i].final_time_predict
...
with open(str(path)+'\\'+str(exp)+'\\grid_regression_random_search_'+str(dataname)+'_'+str(MLSName)+'_'+str(scoringName)+'_'+str(cv)+'_'+str(exp), 'wb') as file:
