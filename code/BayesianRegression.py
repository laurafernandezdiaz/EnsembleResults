...
from Bayesian.my_bayesian_combine import my_bayesian_combine
...
if dataname=='bike_hour':
    X,y = ReadDatasets.load_bike_hourly(seed)    
...
if MLSName=='SVR':
    ...
    MLS=svm.SVR(kernel='linear',verbose=0)
...
cv=3
rep=1
grid_combine=my_bayesian_combine(MLS,GSparams,RG,scoringName,cv,exp,rep,X,y)
...
with open(str(path)+'\\'+str(exp)+'\\grid_regression_bayesian_'+str(dataname)+'_'+str(MLSName)+'_'+str(scoringName)+'_'+str(cv)+'_'+str(exp), 'wb') as file:
