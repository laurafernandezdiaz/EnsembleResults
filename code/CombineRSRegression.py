...
from CombineRS.my_random_combine import my_random_combine
...
cv=3
rep=1
random_combine=my_random_combine(MLS,GSparams,RG,scoringName,cv,exp,rep,X,y,wrapper)
...
with open(str(path)+'\\'+str(wrapper)+'\\'+str(exp)+'\\random_regression_combine_'+str(wrapper)+'_'+str(dataname)+'_'+str(MLSName)+'_'+str(scoringName)+'_'+str(cv)+'_'+str(exp), 'wb') as file:
