...
from GridSearch.my_grid_search import my_grid_search
import pickle
...
grid_search=my_grid_search(MLS,GSparams,RG,scoringName,cv,exp,rep,X,y)


sumfit=0
sumpred=0
for i in range(0,rep):
	sumfit=sumfit+grid_search.grid_search_models[i].final_time_fit
	sumpred=sumpred+grid_search.grid_search_models[i].final_time_predict
print("Time fit per rep",sumfit/rep)
print("Time predict per rep",sumpred/rep)

path=pathlib.Path(__file__).parent.absolute()
# Open file to save grid
with open(str(path)+'\\'+str(exp)+'\\grid_regression_grid_search_'+str(dataname)+'_'+str(MLSName)+'_'+str(scoringName)+'_'+str(cv)+'_'+str(exp), 'wb') as file:
