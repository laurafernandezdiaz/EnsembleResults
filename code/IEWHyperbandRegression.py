...
from IEWHyperband.my_hyperband_combine import my_hyperband_combine
...
cv=3
rep=1
grid_combine=my_hyperband_combine(MLS,GSparams,RG,scoringName,cv,exp,rep,X,y)
...
with open(str(path)+'\\'+str(exp)+'\\grid_regression_generalized_'+str(dataname)+'_'+str(MLSName)+'_'+str(scoringName)+'_'+str(cv)+'_'+str(exp), 'wb') as file:
