...
class RBST:
    ...
    def __init__(self,expError=2,eps=1e-15,fit_intercept=True,readjustAtt=True,stop=0,maxIters=100,reg=True):
        """
        Params:
            ...
            stop    : If 0: not top criterium (run until maxIters)
                      If 1: stops where found a coefficient with higher abs value than the previous
            ...
            reg : {True(default),False} if True a regularization over each coefficient in boosting 
                  will be applied. Regularization starts with a factor of 0.5 a decreases the
                  regularization (increases the factor to 1) in each iteration                                                                                                                                                                                  
    ...
    def fit(self,X_Original,Y_Original):
        ...
        while  res>self.eps and it<maxIters and not (self.readjustAtt==False and it>=A):
            ...
            if self.useReg:
                # REGULARIZATION
                reg=it/(it+1)
                print('Ite={} REGULARIZATION:{:6.4f}'.format(it,reg))
            else:
                reg=1 # No regularization
            ...
            for a in range(A):
                if self.readjustAtt or W[a]==0:
                     [CRes,CWNoReg,CbNoReg,P]=self._regression(XT[a],Y)
                     CW=CWNoReg*reg
                     Cb=CbNoReg
                     ...
                     if MinRes==None or MinRes>CRes:
                         ...
                         MinW_Std=CWNoReg*self.std[a]
            self.CoefAbs.append(abs(MinW_Std))
            ...
            self.ChanSigD2.append(self._changeD2Sign(self.OWErr+[MinRes]))
            ...
            if self.stop>=3:
                ...
                if self.stop==7:
                   ...
                   if (prevgmdl<=gmdl):
                       ...
                       print()
            PrevW=MinW
            PrevWNoReg=MinWNoReg
            PrevW_Std=MinW_Std
            PrevMinA=MinA
            PrevMinB=MinB
            PrevMinBNoReg=MinBNoReg
            ...
            P=list(map(lambda x:x[0]*MinW+MinB,XT[MinA]))
            for i in range(len(Y)):
                Y[i]=Y[i]-P[i]

        # END WHILE LOOP
        if self.useReg:
            print('Update W[{}]={} to '.format(MinA,W[PrevMinA]),end='')
            W[PrevMinA]=W[PrevMinA]-PrevW+PrevWNoReg
            print('{} PrevW={} PrevWNoReg={}'.format(W[PrevMinA],PrevW,PrevWNoReg))
            
            print('Update b={} to '.format(b),end='')
            b=b-PrevMinB+PrevMinBNoReg
            print('{}'.format(b))        

        # Store D2
        if len(self.OWErr)>2:
            self.D2Res=self._derive2(self.OWErr)
    ...
    def _derive2(self,Y):
        return self._derive(self._derive(Y))
    ...
    def predict(self,X):
        ...
        P=[0]*E
        for i in range(E):
            P[i]=sum(map(lambda x,c:x*c,X[i],self.coef_))+self.intercept_
        return P
